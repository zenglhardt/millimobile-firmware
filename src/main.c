...
#define ADC_COMMS_CHAN 3 // ADC channel to use for communication
...
void readADC(struct k_work *work) //reads all ADC channels and stores mV outputs in adc_outputs_mv array
{
       //printk("ADC reading[%u]:\n", count++);
        for (size_t i = 0U; i < ARRAY_SIZE(adc_channels); i++) {
                ...
                // TODO: Should this be run here, or in separate timer?
                if (adc_channels[i].channel_id == ADC_COMMS_CHAN) {
                        
                        // Store the comm's ADC value in ring buffer
                        adc_buf[n % N] = adc_outputs_mv[i];
                        n += 1;
                        update_buffer_stats(adc_buf, N, &adc_stats);
                        norm = 100.0 * (adc_outputs_mv[i] - adc_stats.min) / (adc_stats.max - adc_stats.min);
